...
const PROYECTO_COLUMNS =
  "id, titulo, cliente_id, tipo_id, prioridad, responsable_tecnico_id, etapa_desarrollo, etapa_desarrollo_at, etapa_finalizado_at, tecnico_asignado_at, fecha_ingreso, fecha_prometida, bloqueado";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [activosRes, finalizadosRes] = await Promise.all([
      sb
        .from("proyectos")
    ...
    const tecnicoIds = uniq(todos.map((p) => p.responsable_tecnico_id));
    ...
    for (const p of activos) {
      ...
      g.proyectos.push({
        ...
        dias: diasTranscurridos(desdeAsignacion),
    ...
    const activosPorProgramador = Array.from(gruposActivos.values())
      .map((g) => ({
        ...g,
        // Los que llevan más días arriba: es lo primero que se mira en el pase diario.
    ...
    for (const p of finalizados) {
      ...
      g.proyectos.push({
        ...
        dias_tardados: diasTranscurridos(desdeAsignacion, p.etapa_finalizado_at),
      });
    }
