...
export default function PanelProyectosClient() {
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-5 p-4 md:p-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            />
            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#4FAEB2]">Gerencial</p>
          </div>
          <h1 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900">Panel de proyectos</h1>
          <p className="text-sm text-slate-500">
            Estado de la producción y la plata cerrada. El presupuesto es automático: la primera factura de venta
            de cada cliente.
          </p>
        </div>
        <Link
      ...
      {loading ? (
        <div className="py-20 text-center text-sm text-slate-400">Cargando panel…</div>
      ) : err ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            <Kpi
              label="Entregados del mes"
              value={String(data.kpis.entregados_mes)}
              sub={fmtGs(data.kpis.entregados_mes_monto)}
              accent="ok"
            />
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="space-y-2">
              {data.por_asesor.map((a) => (
                <div key={a.asesor} className="grid grid-cols-[1fr_auto] items-center gap-3">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="space-y-1.5">
                {data.por_rubro.slice(0, 10).map((r) => (
                  <div key={r.rubro} className="grid grid-cols-[1fr_auto] items-center gap-2 text-sm">
                    <div className="min-w-0">
                      <div className="flex items-center justify-between gap-2">
